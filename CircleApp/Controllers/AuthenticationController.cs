...
namespace CircleApp.Controllers
{
    public class AuthenticationController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginVM loginVM)
        {
            ...
            bool isAdmin = await _userManager.IsInRoleAsync(existingUser, AppRoles.Admin);
        ...
        public async Task<IActionResult> ExternalLoginCallback()
        {
            // 1. Lấy thông tin từ GitHub (dùng _signInManager)
            ...
            var email = info.Principal.FindFirstValue(ClaimTypes.Email)
                     ?? info.Principal.FindFirstValue("urn:github:user:email");

            if (string.IsNullOrEmpty(email))
            {
                var githubId = info.Principal.FindFirstValue(ClaimTypes.NameIdentifier);
                email = $"{githubId}@github-user.local";
            }
            ...
            if (user == null)
            {
                var newUser = new User()
                {
                    Email = email,
                    UserName = email,
                    FullName = info.Principal.FindFirstValue(ClaimTypes.Name) ?? $"GitHub User {email.Split('@')[0]}",
                    EmailConfirmed = true,
                    Bio = ""
                };
