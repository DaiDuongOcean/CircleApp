...
namespace CircleApp.Data.Services
{
    public class FriendsService : IFriendsService
    {
        ...
        public async Task<List<UserWithFriendsCountDto>> SearchFriendsAsync(string query, int userId)
        {
            var result = await _context.Users
                .Where(u => u.Id != userId &&
                    (u.FullName.Contains(query) || u.UserName.Contains(query) || u.Email.Contains(query)))
                .Select(u => new UserWithFriendsCountDto()
                {
                    User = u,
                    FriendsCount = _context.Friendships
